...
def visualizar_serie(serie):
    ...
    ax.set_title("Serie temporal (2018-2023)")
    ax.set_xlabel("Fecha")
    ax.set_ylabel("Valor")
...
def analizar_residuo(residuo):
    ...
    sns.histplot(residuo_limpio, kde=True, stat="density")
    ...
    with open("output/ej4_analisis.txt", "w") as f:
        ...
        f.write("Test Jarque-Bera (normalidad)\n")
        f.write(f"JB stat: {jb_stat:.4f}\n")
        f.write(f"p-value: {jb_p:.4f}\n\n")

        f.write("Test ADF (estacionariedad)\n")
        f.write(f"ADF stat: {resultado_adf[0]:.4f}\n")
